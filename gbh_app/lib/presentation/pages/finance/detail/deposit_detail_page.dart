...
class DepositDetailPage extends ConsumerWidget {
        ...
        children: [
          ...
          Expanded(
            child: depositPaymentAsync.when(
              data: (response) => _buildPaymentDetails(context, response.data.payment),
          ...
          Row(
            ...
            children: [
              Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('총 예금액', style: AppTextStyles.bodyMedium.copyWith(color: AppColors.divider)),
                  Text(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildInfoCard('만기 예상', [
            _buildDetailRow('만기 예상액', '${NumberFormat('#,###').format(balance * 1.08)}원'),
            _buildDetailRow('예상 이자', '${NumberFormat('#,###').format(balance * 0.08)}원'),
  ...
  String _getExpiryDate(String startDateStr) {
    ...
    final expiryDate = DateTime(startDate.year + 1, startDate.month, startDate.day);
